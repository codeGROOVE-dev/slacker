...
func TestCommitPRCache_RecordAndFind(t *testing.T) {
	c := cache.NewCommitPRCache()

	// Record a PR with a commit
	c.RecordPR("owner", "repo", 123, "abc123")
	...
	prs = c.FindPRsForCommit("owner", "repo", "def456")
	if len(prs) != 0 {
		t.Errorf("expected empty for unknown commit, got %v", prs)
	}

	// Should not find in different repo
	prs = c.FindPRsForCommit("owner", "other-repo", "abc123")
	if len(prs) != 0 {
...
func TestCommitPRCache_MultiplePRsSameCommit(t *testing.T) {
	...
	c.RecordPR("owner", "repo", 123, "abc123")
	c.RecordPR("owner", "repo", 456, "abc123")
...
func TestCommitPRCache_UpdateExistingPR(t *testing.T) {
	...
	c.RecordPR("owner", "repo", 123, "def456")
...
// Note: This test verifies that when we add a new entry, old entries (>10min) are filtered out.
// We can't easily test this without accessing internal fields, so we trust that RecordPR
// implements the cleanup logic correctly (verified through code review).
func TestCommitPRCache_Expiration(t *testing.T) {
	c := cache.NewCommitPRCache()

	// Add an entry
	c.RecordPR("owner", "repo", 456, "new456")

	// Entry should be findable
	prs := c.FindPRsForCommit("owner", "repo", "new456")
	if len(prs) != 1 || prs[0] != 456 {
		t.Errorf("expected [456] for recent entry, got %v", prs)
	}

	// The actual expiration logic is tested in the cache package's unit tests
	// where we have access to internal fields
}
...
func TestCommitPRCache_EmptyCommitSHA(t *testing.T) {
	...
	c.RecordPR("owner", "repo", 123, "")

	// Should not be findable (empty SHAs are ignored)
	prs := c.FindPRsForCommit("owner", "repo", "")
	if prs != nil {
		t.Errorf("expected nil for empty SHA, got %v", prs)
	}

	// Most recent should also return 0 since nothing was recorded
	mostRecent := c.MostRecentPR("owner", "repo")
	if mostRecent != 0 {
		t.Errorf("expected 0 since empty SHA was not recorded, got %d", mostRecent)
	}
}
...
func TestCachePopulationFromTurnclient(t *testing.T) {
	...
	for _, commitSHA := range mockPR.Commits {
		if commitSHA != "" {
			c.RecordPR("owner", "repo", mockPR.Number, commitSHA)
		}
	}

	// Verify all commits are cached
	for i, commitSHA := range mockPR.Commits {
		prs := c.FindPRsForCommit("owner", "repo", commitSHA)
...
func TestMultipleReposIndependence(t *testing.T) {
	...
	c.RecordPR("owner1", "repo1", 111, "abc123")
	c.RecordPR("owner2", "repo2", 222, "abc123")
	c.RecordPR("owner1", "repo2", 333, "abc123")

	// Each repo should only see its own PR
	prs := c.FindPRsForCommit("owner1", "repo1", "abc123")
	if len(prs) != 1 || prs[0] != 111 {
		t.Errorf("owner1/repo1: expected [111], got %v", prs)
	}

	prs = c.FindPRsForCommit("owner2", "repo2", "abc123")
	if len(prs) != 1 || prs[0] != 222 {
		t.Errorf("owner2/repo2: expected [222], got %v", prs)
	}

	prs = c.FindPRsForCommit("owner1", "repo2", "abc123")
...
func TestMostRecentPR(t *testing.T) {
	...
	c.RecordPR("owner", "repo", 100, "commit100")
	time.Sleep(10 * time.Millisecond)
	c.RecordPR("owner", "repo", 200, "commit200")
	time.Sleep(10 * time.Millisecond)
	c.RecordPR("owner", "repo", 300, "commit300")
...
func TestMostRecentPR_WithMultipleCommitsPerPR(t *testing.T) {
	...
	time.Sleep(10 * time.Millisecond)
	c.RecordPR("owner", "repo", 100, "commit2")
	time.Sleep(10 * time.Millisecond)

	// PR 200 with a commit added AFTER PR 100's last commit
	c.RecordPR("owner", "repo", 200, "commit3")
	...
	time.Sleep(10 * time.Millisecond)
	c.RecordPR("owner", "repo", 100, "commit4")
...
func TestTurnclientFallback_CacheHasRecentPR(t *testing.T) {
	...
	c.RecordPR("testorg", "testrepo", 123, "abc123")
	...
	c.RecordPR("testorg", "testrepo", 123, "def456")
...
func TestTurnclientFallback_NoRecentPR(t *testing.T) {
	...
	mostRecent := c.MostRecentPR("testorg", "unknown-repo")
	if mostRecent != 0 {
...
func TestTurnclientFallback_WrongPR(t *testing.T) {
	...
	c.RecordPR("testorg", "testrepo", 100, "commit1")
	c.RecordPR("testorg", "testrepo", 100, "commit2")
	...
	c.RecordPR("testorg", "testrepo", 200, "commit_from_pr_200")

	// Now cache has both PRs
	prs := c.FindPRsForCommit("testorg", "testrepo", "commit_from_pr_200")
	if len(prs) != 1 || prs[0] != 200 {
		t.Errorf("expected to find PR 200, got %v", prs)
	}
}

// TestCommitPRCache_EmptySHA tests handling of empty commit SHA.
func TestCommitPRCache_EmptySHA(t *testing.T) {
	c := cache.NewCommitPRCache()

	// Record a valid PR
	c.RecordPR("owner", "repo", 123, "abc123")

	// Query with empty SHA should return nil
	prs := c.FindPRsForCommit("owner", "repo", "")
	if prs != nil {
		t.Errorf("expected nil for empty SHA, got %v", prs)
	}
